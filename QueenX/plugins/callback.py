...
from QueenX import YouTube, app
from QueenX.core.call import Queen
from QueenX.misc import SUDOERS, db
from QueenX.utils import bot_sys_stats
from QueenX.utils.database import (
    get_active_chats,
    get_lang,
    is_active_chat,
    is_music_playing,
...
from QueenX.utils.formatters import seconds_to_min
from QueenX.utils.inline import (
    stream_markup,
    stream_markup_timer,
    telegram_markup,
...
from QueenX.utils.stream.autoclear import auto_clean
from QueenX.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Queen.pause_stream(chat_id)
        ...
        await Queen.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await CallbackQuery.answer()
        await Queen.stop_stream(chat_id)
        ...
        try:
            ...
            try:
                ...
                return await Queen.stop_stream(chat_id)
            except:
                return
        await CallbackQuery.answer()
        queued = check[0]["file"]
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await Queen.skip_stream(chat_id, link, video=status, image=image)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            if videoid == "telegram":
                ...
                try:
                    ...
                    image = None
            try:
                await Queen.skip_stream(chat_id, queued, video=status, image=image)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
